...
class xtfxMorningPipeline(object):
    ...
    def process_item(self, item, spider):
        data = dict(item)
        insert_sql = "insert into xtfx_morning(question, questionImg, optiona, optionb, optionc, optiond, answer, answeranalysis, field) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
        try:
            # 执行sql语句
            self.cursor.execute(insert_sql, tuple(data.values()))
            # 提交到数据库执行
...
class xxxtxmMorningPipeline(object):
    ...
    def process_item(self, item, spider):
        data = dict(item)
        insert_sql = "insert into xxxtxm_morning(question, questionImg, optiona, optionb, optionc, optiond, answer, answeranalysis, field) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"
        try:
            # 执行sql语句
            self.cursor.execute(insert_sql, tuple(data.values()))
            # 提交到数据库执行

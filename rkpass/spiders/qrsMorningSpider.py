...
import scrapy
import re
from rkpass.items import qrsMorningItem

# 嵌入式系统设计师上午题
class QrsmorningspiderSpider(scrapy.Spider):
    name = 'qrsMorningSpider'
    allowed_domains = ['www.rkpass.cn']
    start_urls = []
    paperId_list = ['545', '455', '270', '269', '268', '267', '266', '265']  # 试卷的所有ID
    field_list = ['20162', '20152', '20142', '20132', '20122', '20112', '20102', '20092', ]  # 跟上行试卷所有ID对应考试场次

    for j in range(len(paperId_list)):
        for i in range(1, 76):
            start_urls.append(
                'http://www.rkpass.cn/tk_timu/13_' + str(paperId_list[j]) + '_' + str(i) + '_xuanze.html?field=' +
                field_list[j])

...
import scrapy
import re
from rkpass.items import wlghMorningItem

class WlghmorningspiderSpider(scrapy.Spider):
    name = 'wlghMorningSpider'
    allowed_domains = ['www.rkpass.cn']
    start_urls = []
    paperId_list = ['535', '470', '380', '288', '289', '292', '293', '294', '295', '313']  # 试卷的所有ID
    field_list = ['20172', '20162', '20152', '20142', '20132', '20122', '20112', '20102', '20101', '20092']  # 跟上行试卷所有ID对应考试场次

    for j in range(len(paperId_list)):
        for i in range(1, 76):
            start_urls.append(
                'http://www.rkpass.cn/tk_timu/5_' + str(paperId_list[j]) + '_' + str(i) + '_xuanze.html?field=' +
                field_list[j])

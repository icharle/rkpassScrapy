...
import scrapy
import re
from rkpass.items import xxaqMorningItem


# 信息安全工程师上午题
class XxaqmorningspiderSpider(scrapy.Spider):
    name = 'xxaqMorningSpider'
    allowed_domains = ['www.rkpass.cn']
    start_urls = []
    paperId_list = ['635', '567', '502', '479']  # 试卷的所有ID
    field_list = ['20191', '20181', '20171', '20162']  # 跟上行试卷所有ID对应考试场次

    for j in range(len(paperId_list)):
        for i in range(1, 76):
            start_urls.append(
                'http://www.rkpass.cn/tk_timu/18_' + str(paperId_list[j]) + '_' + str(i) + '_xuanze.html?field=' +
